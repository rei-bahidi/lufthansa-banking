...
@pytest.mark.django_db
def test_transfer_with_inactive_from_account(inactive_account):
    ...
    with pytest.raises(ValidationError) as excinfo:
        Transaction.objects.create(transaction_type='TRANSFER', amount=Decimal('150.00'), from_account=inactive_account, to_account=account2)
    assert excinfo.value == ValidationError("The 'from_account' is not active.")
...
@pytest.mark.django_db
def test_credit_transaction_validation_without_account(active_currency):
    with pytest.raises(ValidationError) as excinfo:
        Transaction.objects.create(transaction_type='CREDIT', amount=Decimal('100.00'), currency=active_currency)
    assert excinfo.value == ValidationError("A credit transaction requires a 'to_account'.")

@pytest.mark.django_db
def test_transfer_transaction_validation_without_accounts(active_currency):
    with pytest.raises(ValidationError) as excinfo:
        Transaction.objects.create(transaction_type='TRANSFER', amount=Decimal('100.00'), currency=active_currency)
...
def test_process_balance_updates_debit(active_account, active_currency):
    Transaction.objects.create(transaction_type='DEBIT', amount=Decimal('100.00'), from_account=active_account, currency=active_currency)
...
def test_process_balance_updates_credit(active_account, active_currency):
    Transaction.objects.create(transaction_type='CREDIT', amount=Decimal('200.00'), to_account=active_account, currency=active_currency)
    active_account.refresh_from_db()
...
def test_process_balance_updates_transfer(active_account, active_currency):
    account2 = Account.objects.create(balance=Decimal('500.00'), currency=active_account.currency, user=active_account.user, is_active=True)
    Transaction.objects.create(transaction_type='TRANSFER', amount=Decimal('150.00'), from_account=active_account, to_account=account2, currency=active_currency)
    active_account.refresh_from_db()
...
def test_insufficient_funds_debit(active_account, active_currency):
    with pytest.raises(ValueError) as excinfo:
        ...
        assert str(excinfo.value) == "Insufficient funds in 'from_account'."
...
def test_amount_too_small_credit(active_account):
    with pytest.raises(ValueError) as excinfo:
        ...
        assert str(excinfo.value) == "The amount is too small."
...
def test_amount_exceeds_limit_credit(active_account):
    with pytest.raises(ValueError) as excinfo:
        ...
        assert str(excinfo.value) == "The amount exceeds the 10,000 limit."